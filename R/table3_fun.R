...
general_table3<-function(x,y,split_var,adj,data,div=c(),totalcount=T){
  ...
  ###split_var本身为连续型###
    ...
    if(length(div)==0){
      split_var_factor<-div_quantile(split_var,2,data)#默认将分割变量按中位数转成因子型
      divdata<-data#替换成数据框
      divdata[,split_var]<-factor(split_var_factor)
      sdata<-split(divdata,divdata[,split_var])#根据分割变量分组
      #计算交互作用p值
      ...
      split_var_factor<-div_custom(split_var,div,data)#将分割变量转成因子型
      divdata<-data#替换成数据框
      divdata[,split_var]<-factor(split_var_factor)
      sdata<-split(divdata,divdata[,split_var])#根据分割变量分组
      #计算交互作用p值
  ...
  # dataframe<-as.data.frame(lapply(dataframe,as.numeric))#将dataframe中的元素转化为数值型
  if(is.factor(ss)==TRUE){
...
logistic_table3<-function(x,y,split_var,adj,data,div=c(),totalcount=T){
  ...
  j_len<-ifelse(length(levels(xx))==0,2,length(levels(xx)))
  for (i in 1:length(sdata)){
    model<-stats::glm(formula_adj, data = sdata[[i]], family = stats::binomial(link = "logit"))#分组做logistic回归
    model<-summary(model)
...
cox_table3<-function(x,y_time,y_factor,split_var,adj,data,div=c(),totalcount=T){
  ...
  j_len<-ifelse(length(levels(xx))==0,2,length(levels(xx)))
  for (i in 1:length(sdata)){
    model<-survival::coxph(formula_adj, data = sdata[[i]])#分组做cox回归
    model<-summary(model)
